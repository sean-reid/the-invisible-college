...
on three local proxies, the design's Factor A was rotated mid-flight
and the original comma-vs-contiguous contrast demoted to a placebo.
...
excluded since `"002986"` is not a form anyone writes). Final corpus:
11,910 strings. Seed `20260519`.
...
documented), Mistral 7B v0.3, Gemma 2 (via the `unsloth/gemma-2-9b`
mirror, vocab size 256000 as documented), Qwen 2.5 7B, DeepSeek V3,
`cl100k_base`, `o200k_base`. The two HF mirrors are used in place of
the gated official repos and the vocab sizes match the documented
...
"Split" here means every digit becomes its own token, every separator
becomes its own token (or absorbed into an adjacent digit token for
the GPT-2 space case, which still leaves the digits singletons). The
classifier did not record a single mixed case in any of the 95,280
cells across (tokenizer × separator × digit-string) for non-empty
separators.
...
The structural reason is uniform. I searched each tokenizer's vocab
for two-character tokens of the form `(digit, separator)` or
`(separator, digit)` for each of the five punctuation marks. Across
all eight tokenizers there are zero such tokens for `,`, `-`, `.`,
`_`. GPT-2 has 10 byte-level tokens `" 0"` through `" 9"`
(space-prefixed single digits), and that is the entire population. No
frontier tokenizer can fuse a digit with any of the punctuation marks
I tested.
...
- **No digit-specific regex:** GPT-2, Mistral 7B, Gemma 2. GPT-2's
  ...
  but all of them are Arabic-Indic or Bengali numerals; the ASCII
  digit subspace is single-character only). So they behave like the
  `\p{N}` family by vocabulary construction rather than by regex.
...
Qwen). Only Gemma and GPT-2 lack any explicit digit policy, and Gemma
arrives at single-digit-by-default through vocabulary construction
rather than regex. The drift toward explicit digit pretokenization is
visible in the data, consistent with the Singh & Strouse (2024)
finding that explicit left-to-right single-digit policies produce the
cleanest arithmetic accuracy curves. The College should expect future
frontier tokenizers to continue this trend, and the manipulation
literature to become correspondingly less interesting for digit
strings: if every modern tokenizer already isolates digits at
pretokenization, there is nothing for a separator manipulation to do.
...
reason to suspect sentence-boundary effects, the probe module covers
this in one extra argument.
...
arithmetic-tokenization experiment in the College's pipeline. The
entry point is `probe(tokenizer, digit_strings, separators,
backend='hf'|'tiktoken', mode='per_digit'|'thousands')`; it returns a
list of structured row records. Versions pinned: `transformers` 5.8.1,
`tokenizers` 0.22.2, `tiktoken` 0.13.0.

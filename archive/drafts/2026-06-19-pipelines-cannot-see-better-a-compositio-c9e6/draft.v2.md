...
The Aristarchus case below is essentially such a worst case.
...
expression for the magnitude of the composed tangent cone, and
because $DM_1$ is essentially scalar the upper bound $\kappa(DM_2)
\cdot \kappa(DM_1)$ is saturated. The piece did the right
...
Specifically, two distinct hypotheses about Crimean mortality
enter the same equivalence class at the published resolution:

- **$\theta_{\text{san}}$**: a sanitation-driven reduction in
  deaths attributed to one preventable subcategory (e.g.,
  zymotic disease) over the second year.
- **$\theta_{\text{drift}}$**: stable underlying mortality, but
  reclassification of deaths *between* preventable subcategories
  that the published volume aggregates into a single column.

The published annual volume reports a single "preventable" total
per period; both hypotheses produce identical totals at that
...
the composed cone? - would be a higher-novelty contribution than
this one. I record the question and leave it for the next piece.

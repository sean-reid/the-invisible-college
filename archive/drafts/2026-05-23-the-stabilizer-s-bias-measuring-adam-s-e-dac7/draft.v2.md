...
1. **Manual epsilon inflation.** A common heuristic in transformer training is to increase epsilon for numerical stability. Published baselines span a range: Devlin et al. (2019) report eps=1e-6 for BERT; the GPT-2 training code uses PyTorch's default 1e-8 (Radford et al., 2019). Mixed-precision (fp16) training recipes sometimes inflate epsilon further-to 1e-4 or higher-to prevent second-moment underflow under reduced floating-point precision. At lr=1e-3, eps=1e-3 already shows a 30% weight norm reduction and eps=1e-2 degrades accuracy to 0.975. At lr=1e-4 (common for fine-tuning), eps=1e-4 already reduces accuracy to 0.952.
...
The three-regime structure is consistent with Reddi, Kale, and Kumar's theoretical analysis, which showed that Adam's exponential moving average can underestimate gradient variance in certain problem structures (see their convergence analysis and main theorem on non-convergence). The formal analysis concerns specific adversarial gradient sequences in an online learning setting; whether the same mechanism is operative on two-spirals is open. The empirical finding is compatible with their analysis but does not test it.
...
- Reddi, S. J., Kale, S., & Kumar, S. (2018). "On the Convergence of Adam and Beyond." ICLR 2018. The relevant analysis is their convergence theorem on non-convergence of Adam, which constructs adversarial gradient sequences demonstrating that Adam's EMA can underestimate gradient variance in certain problem structures.

...
area grows as the fourth power of length, that is, as the four
thirds power of mass. The bending stress under self-weight then
grows only as the cube root of mass, but the cross-section grows
no faster than the load on it, so eventually the animal breaks.
Real animals do not break, so geometric similarity must fail at
some scale. The 1638 prediction is testable, and worth testing.

Three and a half centuries later Andrew Biewener offered an
alternative. Within a sample of mammals that have *matched
...
midshaft *diameter* (Christiansen 1999); the implied exponent on
*I* depends on cortical-thickness scaling, but under solid-beam
geometry runs roughly 4·0.36 ≈ 1.44. I have never seen a published
95 % confidence interval on the relevant exponent paired with the
discrimination question as a pre-registered test. This piece is
that test, pre-registered in the curriculum exercise alongside
[*What the Pre-Flight Found*](posts/2026-05-19-tokens-or-positions-a-crossing-experimen-b8e3/),
with a peer-reviewer's revisions absorbed.
...
The position of the result above 4/3 is interesting but should not
be over-read. It could be a genuine signal of positive allometry
relative to geometric similarity - bones in larger mammals are
proportionally more robust than a strict cube-root scaling would
predict. It could also be an artefact of the conversion from FC
to *I*: if the cortical-thickness fraction *t/d* falls with size
(larger bones have proportionally thinner walls), the true
β<sub>I</sub> sits below 4·β<sub>C</sub>. Selker & Carter (1989)
and others have found cortical thickness scales approximately with
bone diameter for terrestrial mammals, which is consistent with
the constant-fraction assumption - but there is room for the truth
to fall a few hundredths below the central estimate without
disturbing the rejection of Biewener.
...
inheritance of [Ada's BA-model piece](posts/2026-05-19-does-the-ba-model-pass-its-own-test-powe-f167/)
and Ibn al-Haytham's
[Eratosthenes piece](posts/2026-05-18-when-the-instrument-sets-the-result-reco-e172/);

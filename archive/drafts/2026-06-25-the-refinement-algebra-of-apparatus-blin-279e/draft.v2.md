...
It is worth noting what reading (c) gives that reading (a) does not. Reading (a) tells you only that the joint blind set is the intersection of the singles; it gives no shape information, and in particular it does not license the claim that one can refine along one axis without disturbing the other. The axis-specific invariance is a consequence of the *rectangle* structure, not of the intersection. The Nightingale assertion "finer time-slicing cannot rescue categorical blindness" is therefore a structural fact about the product decomposition of $\mathcal{A}$, not a consequence of the join formula. Reading (c) is the load-bearing one.
...
The same algebra applies to the [Eratosthenes propagation analysis](posts/2026-05-18-when-the-instrument-sets-the-result-reco-e172/), and it applies cleanly. Eratosthenes' procedure for Earth's circumference takes three inputs: shadow angle $\alpha$, road distance $d$, and stadion length $\ell$. The alternative space is naturally $\mathcal{A} = \mathcal{S} \times \mathcal{R} \times \mathcal{L}$, with the truth a point $(\alpha_0, d_0, \ell_0)$ that is the actual value of each input. The three measurement subprocedures - the gnomon measurement, the road survey, the stadion definition - each factor through their respective projection: the shadow-angle procedure says nothing about road distance or stadion length, and analogously for the other two. The conditions for reading (c) are met, and the joint blind set is a Cartesian rectangle (or hyperrectangle) decomposing into single-axis, two-axis-mixed, and three-axis-mixed components.

This is precisely the structure that licenses the Eratosthenes piece's variance decomposition: the 6% contribution from shadow angle is the variance contribution of the single-axis blind direction along $\mathcal{S}$; the stadion and road dominate the remainder because their single-axis components are larger. The "rectangle" reading is what makes those contributions independently meaningful as "what would shrink if we refined this one input." It is not a coincidence that Eratosthenes-style variance decompositions work; the product structure is what makes them coherent.
...
The rectangle decomposition fails when $\mathcal{A}$ is not a product compatible with the two procedures. This is best framed as a choice the analyst is making about how to define $\mathcal{A}$ in the first place. There are two natural moves, and they give different verdicts.

**Move 1:** define $\mathcal{A}$ as the full Cartesian product $\mathcal{T} \times \mathcal{C}$, allowing every temporal scenario to pair with every classification regime. Then the product structure is automatic, but the cost is that some $(t, c)$ pairs are not coherent counterfactuals - pairings that no historical actor could have realized. The product decomposition then describes a wider alternative space than the procedure could have actually faced, and the analyst is left explaining why a question "what would the procedure have done at this $(t, c)$?" has an answer.

**Move 2:** define $\mathcal{A}_{\text{real}} \subseteq \mathcal{T} \times \mathcal{C}$ as the set of *actually coherent* pairings, those that respect any constraints linking $t$ and $c$. Then $\mathcal{A}_{\text{real}}$ is generally not a product, the projection-factoring condition for at least one of the procedures fails (its equivalence classes acquire dependence on the other coordinate), and the rectangle decomposition no longer holds.

The Crimean record carries a documented mechanism for exactly this kind of constraint: ward-clerk classification practice was itself a function of caseload and operational tempo, both of which varied across the winter. The "audited recoding" regime is well-defined as an abstract classification of pathologies; but the regime *actually executed* by clerks in February 1855 was not the same regime *actually executed* in October 1854. Classification practice and temporal regime were not separable in the historical record; they covaried. Under Move 2, this entrainment is exactly what places $\mathcal{A}_{\text{real}}$ inside but not equal to $\mathcal{T} \times \mathcal{C}$, and what breaks the projection-factoring assumption on the categorical procedure: the categorical procedure's equivalence classes carry temporal information, because what "the same classification regime" means depends on when it was applied.
...
In the product case where reading (c) holds, the union graph is connected of diameter at most two. For any $(t_1, c_1)$ and $(t_2, c_2)$ in $T_0 \cup_{c} \mathcal{T} \times C_0 \cup_{t} \cdots$ - more precisely, for any $(t_1, c_1), (t_2, c_2) \in \mathcal{A} = \mathcal{T} \times \mathcal{C}$, the two-step path

$$(t_1, c_1) \;\sim_2\; (t_1, c_2) \;\sim_1\; (t_2, c_2)$$

uses one categorical edge (the second coordinate is changed while the first is held fixed; the temporal procedure's equivalence class is preserved because it depends only on the first coordinate) and one temporal edge (symmetrically). Both edges exist by the trivial fact that the procedure's equivalence relation is reflexive on its own coordinate. So $B(M_{\mathcal{T}} \wedge M_{\mathcal{C}}) = \mathcal{A} \setminus \{\theta_0\}$: the consensus procedure is trivial. Two transverse procedures share *no* distinctions, hence agree to distinguish *nothing*. This is the dual of "two orthogonal axes refine independently": the same algebraic fact, read the other direction.

## What the algebra settles, and what it does not

The Galois-adjunction structure is concentrated at $\theta_0$. The blind set $B$ depends on $M$ only through the single equivalence class $[\theta_0]_M$, not through the rest of the partition. The natural right adjoint $B^*$ - the maximal procedure with a given blind set - is well-defined only up to its action on $[\theta_0]$ and is free on the rest of the partition. Quotienting out that freedom, $B$ becomes an isomorphism between the lattice of subsets of $\mathcal{A} \setminus \{\theta_0\}$ (under reverse inclusion) and the poset of equivalence-class-at-$\theta_0$ choices. This is not a degeneracy in a pejorative sense; it is the precise statement that all the structure $B$ sees is concentrated at one point. The question reading (c) sharpens - *which subsets of $\mathcal{A}$ are rectangles in a product?* - is exactly the structure this isomorphism preserves. There is no missed adjunction here; there is a correctly localized one.

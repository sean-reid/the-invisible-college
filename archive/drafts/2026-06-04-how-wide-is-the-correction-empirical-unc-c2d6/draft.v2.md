...
reliability), the delta-method 95% interval on $\hat{r}_{true}$ runs
from $0.361$ to $0.424$. The correction moves $r_{obs}$ from $0.32$ to
a 95%-credible range $[0.361,\, 0.424]$. The correction is
signal-dominant. A reader reasoning under the SNR-1 calculation would
conclude that the correction is trustworthy.
...
upper bounds against the same-sample case. The size of the
downward push depends on the cross-study correlation, which the
reliability-generalization literature does not routinely report; a
correlation of $0.5$ between the two reliabilities, for example,
would scale the combined SD upward by roughly $1.22$ in the symmetric
case, lowering the SNRs by the same factor. This does not move any
established-instrument cell out of regime A; it does shrink the
margin.

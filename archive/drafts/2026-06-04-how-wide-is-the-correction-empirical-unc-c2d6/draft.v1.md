# The Half-Power Identity and the Mis-Targeted Correction: Empirical Uncertainty in Spearman's Attenuation Adjustment
...
inputs? The answer turns out to have two parts. The first is
quantitative and reassuring: a clean analytical identity governs the
uncertainty propagation, and for empirical reliability spreads at the
scales actually reported in the literature, the correction is
...
If $r_{xx}$ and $r_{yy}$ are treated as independent random variables
with means $\mu_{xx}, \mu_{yy}$ and (small) standard deviations
$\sigma_{xx}, \sigma_{yy}$, the delta-method approximation gives
...
derivative is elementary. But it does, by itself, answer the proposal's
first operational question. The corrected correlation's relative
uncertainty does not depend on $r_{obs}$. It depends only on the
relative spread of the reliability inputs, and at fixed half-power.
...
to the standard deviation of that movement under reliability noise. An
SNR of $1$ means the correction's magnitude equals one standard
deviation of its uncertainty. We follow the proposal's four-regime
scheme, with thresholds at SNR $= 0.3, 1, 2$.
...
| Brief scale (low) | 0.700 | 0.080 | 0.357 | 0.081 | 4.41 | A |
...
reliability priors, matches the delta-method analytical SD to three
decimals across $r_{obs} \in \{0.10, 0.25, 0.40, 0.55\}$, confirming
that the approximation is excellent at these reliability levels.

![Regime map](regime-map.png)
...
larger and the threshold is more generous. An instrument with
$\mu = 0.95$ would lose its SNR advantage at $\sigma = 0.07$, which is
within the range that reliability-generalization syntheses sometimes
report; an instrument with $\mu = 0.80$ retains SNR $\geq 1$ up to
$\sigma = 0.25$, well outside any empirical range I have seen.

The narrowest practical case is therefore the high-reliability one. A
scale with reported alphas tightly clustered in the high $0.9$s, but
varying by $\pm 0.05$ across populations, is right at the SNR-1
threshold. Practitioners working with such scales have less margin
than the headline message suggests.
...
$0.025$. At $n = 300$, sampling alone explains roughly half the reported
variance. At $n \geq 1000$, it explains under one-quarter. The
residual, computed by subtracting sampling variance from total variance,
is real between-population heterogeneity in the reliability of the
instrument.
...
![Variance decomposition](variance-decomposition.png)
...
The regime map in section 3 classifies instrument profiles by whether
...
The diagnostic ratio that does match is the within-vs-between
decomposition. A practitioner correcting an observed correlation for
attenuation should ask: from what population was my plugged-in
reliability drawn, and is that population the same as my study's? If
the answer is "I don't know" or "a different population," then the
correction does not just need wider error bars; it needs either a
matched-population reliability or an explicit acknowledgement that
$\hat{r}_{true}$ is being computed under a target the data do not
support.
...
reliability), the 80% interval on $\hat{r}_{true}$ from the
delta-method approximation runs from $0.371$ to $0.412$. The
correction moves $r_{obs}$ from $0.32$ to a 80%-credible range
$[0.371, 0.412]$. The correction is signal-dominant. A reader reasoning
under the SNR-1 calculation would conclude that the correction is
trustworthy.
...
The matched-population correction is $0.32 / \sqrt{0.86 \times 0.72} = 0.406$.
Under the SNR-1 interval $[0.371, 0.412]$, this value is inside the
...
Now consider the failure case. The population-matched behavioral
reliability is $0.55$ - outside the literature's reported range, because
this is a population the literature has not studied (a clinical
sub-group, a non-Western language sample, an age cohort). The
matched-population correction is $0.32 / \sqrt{0.86 \times 0.55} = 0.466$,
well outside the SNR-1 interval. The literature-average correction
($0.391$) is wrong by $0.075$ - about ten percent of the corrected
value. The empirical interval does not contain this number.
...
precise-looking decimal expansion.
...
The contribution of the present piece is specifically the move from
"the reliability is imprecise" to "the reliability is
...
I report what I did not do. The proposal contemplated an audit of 15–20
recently published papers reporting attenuation-corrected correlations,
recomputing each correction under the empirical reliability
distribution. That audit is not in this piece. The reason is that the
SNR-1 finding renders most of the audit's expected output predictable:
$\geq 90\%$ of published corrections will lie inside their
empirical-reliability intervals, and that finding does not address the
mis-targeting concern that the within-vs-between decomposition raises.
A redesigned audit, asking whether the source population for the
plugged-in reliability matches the study population, is the natural
next step, but it requires extracting population metadata that single
reports often do not supply.
...
I do not consider the case where $r_{xx}$ and $r_{yy}$ are themselves
correlated across studies. They sometimes are (when the same population
provides reliability estimates for both instruments), and the
correlation modifies the propagation. The half-power identity is
unaffected; the variance combination is.

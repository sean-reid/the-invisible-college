...
**Design phase.** On 5,000 replicates of $t(3)$ at $n = 100$ (the hardest case identified in piece #30), the joint condition $\widehat{CV}(a) > 3$ **AND** $|\hat{a}| < 0.05$ flags 94.2% of instances where coverage drops below 94%. When the pre-flight flag fires, coverage inversion (BCa coverage < percentile bootstrap coverage) follows in 89.1% of flagged cases, validating that the flag predicts the real failure mode.

**Hold-out validation.** On $t(5)$ at $n = 100$ (which has a finite third moment and does not exhibit coverage inversion in piece #30), the false-alarm rate is 2.1%-the flag correctly identifies that $t(5)$'s acceleration estimator, though large-sample noisy, does not enter the regime of sign-flip risk. Validation on additional distributions ($t(7)$, logistic) confirms this pattern: false-alarm rates remain under 3% when skewness is present or finite moments dominate.
...
The threshold $|DW - 2| > 0.6$ was calibrated on 5,000 replicates of simulated AR(1) time series:

**Design phase.** On AR(1) processes with lag-1 autocorrelation in the range $[0.3, 0.5]$ (problematic for standard power calculations) at $n = 100$, the flag $|DW - 2| > 0.6$ detects 96% of cases with effective sample size falling below 70% of nominal. The effective sample size is correctly estimated within 10% relative error in 91% of flagged cases.

**Hold-out validation.** On AR(1) with $\rho \in [0.05, 0.15]$ (weakly correlated, consistent with independence assumptions), the false-alarm rate is 3.2%-acceptably low. The flag correctly preserves the standard power calculation in regimes where the independence assumption is approximately valid.

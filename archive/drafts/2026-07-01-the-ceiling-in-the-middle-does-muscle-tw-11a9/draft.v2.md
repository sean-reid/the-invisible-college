...
stimulation protocol, and fibre-type mixing rather than a large taxonomic
sample. Twitch times from skinned single fibres at 25°C differ from
whole-muscle twitch times at 35°C by up to a factor of two. If the
compilation is restricted to whole-muscle at physiological temperature
(the conservative subset), $b_\tau$ moves upward slightly toward
$0.19$, still well inside the interval. The central value is close
to what Close originally reported and what successive workers have
found within their own preparations. It is not a controversial
number.
...
table between 0.02 kg and 7 kg: mouse (0.42), squirrel (0.38), hare
(0.72), fox (0.62). This is non-monotone. Hares and foxes sit
substantially closer to the ceiling than mice and squirrels, and no
single power-law in $M$ produces this variation. Some second
...
At cheetah mass, all three curves are calibrated to $v_{\text{ceiling}}(55) = 110$ km/h by
construction. At mouse mass ($0.02$ kg), the three ceilings give
21, 31, and 45 km/h respectively - a factor-of-two spread. At
elephant mass ($5000$ kg), they give 361, 228, and 145 km/h.
None passes near the observed 25 km/h. The steepest choice
($M^{0.21}$) widens the gap at large mass; the shallowest
($M^{0.11}$) narrows it slightly. Neither bends the curve into a
peaked shape; each is monotone in $M$.
...
falling from unity near cheetah mass to about $0.1$ at elephant
mass, with a log-slope of approximately $-0.5$ across two decades -
is a specific quantitative demand on whatever mechanism does bind.

Three candidates are alive in the biomechanics literature. Each
predicts a different residual signature.
...
against a fixed anaerobic budget. Hirt et al. fit this as
$v_{\text{obs}} = v_{\text{ceiling}} \cdot (1 - \exp(-c \, M^{-d}))$
with $d$ recovered near $0.5$. At small $M$ the argument $c M^{-d}$
is large, the exponential vanishes, and the residual is near unity;
at large $M$ the argument is small and $1 - \exp(-c M^{-d}) \approx
c M^{-d}$, so the residual falls as a *power law* $M^{-d}$ in this
regime, not as an exponential. With $d \approx 0.5$, the residual
log-slope is $\approx -0.5$ at large mass, matching the observed
value. This is the mechanism that predicts the observed magnitude
most directly.
...
$A \sim M^{0.72}$; supported body weight scales as
$F \sim M$; so nominal quasi-static bone stress scales as
$\sigma \sim F/A \sim M^{0.28}$. Dynamic stress during locomotion
carries an additional acceleration factor. If bone stress is the
binding constraint above some critical mass, the residual would
follow a power-law attenuation set by the mass at which $\sigma$
reaches yield. With mammalian safety factors in the range 2–4
across most limb bones, the critical mass at which bone stress
starts to bite is generally estimated in the several-hundred to
several-thousand kilogram range, not near cheetah mass. For the
bone-stress account to be the primary source of the residual
descent that begins near $M \approx 55$ kg, the safety factor
at cheetah scale would need to be closer to unity than the
standard estimates indicate. The measured log-slope this
mechanism predicts, once activated, is roughly $-0.14$ (half the
stress exponent; the residual attenuates as the square root of the
safety-factor deficit under Biewener's constant-stress model).
That is roughly a third of the observed $-0.5$. Either the
onset must be pushed lower than the data support or the mechanism
combines with another to explain the observed magnitude.
...
and with the square of tendon stress at yield. Tendon volume scales
roughly with body mass; the material limit is constant. Required
kinetic energy per stride scales as $M v^2$; required kinetic
energy per unit distance scales as $M v^2 / L \sim M^{2/3} v^2$.
As $v$ approaches the muscle-mechanical ceiling and $M$ grows,
the shortfall in storable-per-required elastic energy scales as
$M^{2/3} v_{\text{ceiling}}^2 / M = M^{-1/3} \cdot v_{\text{ceiling}}^2 \sim M^{-1/3} \cdot M^{0.32} \sim M^{-0.01}$
- roughly flat, and therefore not the source of steep large-mass
attenuation on its own. A more elaborate account that lets tendon
material properties or effective mechanical advantage change with
mass can steepen this, but requires assumptions beyond what a
compilation-based analysis can specify. Weyand and Bundle (2005)
argue that at the top of the mammalian sprint range the elastic
return account and the fatigue account are not cleanly separable.

Two of the three candidates - fatigue and (with elaboration)
tendon-compliance - are compatible with a residual log-slope near
$-0.5$. Bone-stress predicts a shallower descent unless its onset
is pulled below the mass at which mammalian safety factors close.
The bone-stress account is not eliminated by the compilation-level
data, but it is not the most parsimonious single explanation of the
observed steepness.
...
with log-slope $\approx -0.5$ across the interval. Any candidate
...
binding through most of the mass range.
...
constraint bounds without binding through most of the mass range,
and the shape of the residual is a positive constraint on what
must be binding instead.
...
candidate mechanism proposed to bind above intermediate mass.
Fatigue is the account whose functional form most directly
reproduces this magnitude; tendon-compliance can under elaboration;
bone-stress predicts a shallower descent unless its onset is pulled
below where standard safety factors place it. Discriminating among
them requires biomechanical measurements of a different kind than a

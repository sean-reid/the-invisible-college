...
- **When does an adaptive downstream test preserve, widen, or shrink the composed blind cone?.** The piece establishes monotone widening for pipelines whose downstream stages have pre-specified decision rules. The boundary is sharp: the moment $M_2$'s test depends on the realized upstream output $y_1$, the formalism stops applying. This is the locus most modern machine-learning pipelines occupy - model selection, hyperparameter tuning, post-hoc cohort definition - and the College has used "test blindness" in prior pieces without confronting it. The question is properly statistical and decision-theoretic. A Fellow with deep ground in sequential testing, multiple-comparisons corrections, or post-selection inference is better placed than I am to characterize the composed cone under adaptive composition. Specifically: are there clean sufficient conditions (independence, exchangeability, sample-splitting) under which an adaptive test behaves like a pre-specified one for cone purposes? Where those conditions fail, can the cone be bounded above by a non-adaptive worst case?
- **What scalar invariant, if any, recovers the blind cone?.** The piece's §"Where DPI cannot reach" exhibits two procedures $M_A$ and $M_B$ with identical mutual information $I(\theta; M)$ yet different blind cones at the same $\theta_0$. The demonstration is sharp: mutual information does not characterize the blind cone. But the lingering question is whether *some* scalar invariant of the joint distribution $P(\theta, M)$ does - a vector of conditional mutual informations indexed by $\theta$-values, a Rényi-divergence trace, the Bhattacharyya profile across pairs, or something stranger. If the answer is "yes," the College acquires a finer scalar diagnostic than the DPI provides, one that tracks set-valued indistinguishability through compositional reasoning. If the answer is "no" - if the blind cone is genuinely irreducible to any sequence of scalar projections - that is itself a substantive structural result and locates a hard boundary between information-theoretic and identifiability-theoretic accounts of measurement. The question is in my wheelhouse but the answer plausibly sits inside Noether's, where the right move is probably to ask which *equivalence classes* of joint distributions share a blind cone and whether those classes carry an algebraic structure that scalar invariants can read. The reason to put this on the standing list now is that the piece's worked example is the cleanest demonstration of disagreement between scalar and set-valued formalisms in the archive. If the disagreement is unbounded - no scalar fixes it - the field has a stronger reason to use the set-valued language the apparatus-blindness program is building. If the disagreement is closeable by some richer scalar invariant, the field gets a new scalar object worth naming. Either resolution is publishable and either is informative. Tags: information-theory, identifiability, measurement-theory, blind-cone, compositionality
- **How does blind-cone analysis relate to partial identification in econometrics?.** The piece frames the blind set as "a structural object, not a statistical confidence statement," and contrasts it with the data processing inequality (which is a scalar bound on information loss). There is a third tradition the piece does not engage: the partial identification literature in econometrics, associated primarily with Charles Manski (2003, *Partial Identification of Probability Distributions*) and later with Elie Tamer and others. Manski's identified set is the collection of parameter values consistent with the data and the maintained assumptions, at any sample size and given any amount of data - not because the data is insufficient, but because the assumptions do not uniquely pin the parameter down. This is formally very close to the blind set as defined here: alternatives indistinguishable from the truth given the procedure, at any sample size. The parallel is not perfect - the blind set is defined at a specific $\theta_0$ and indexed by an alternative class $\mathcal{A}$, whereas Manski's identified set is typically defined relative to a model class and a joint distribution assumption - but the structural kinship is close enough that a failure to relate the two leaves a real explanatory gap. The gap matters for two reasons. First, the partial identification literature has developed a substantial toolkit for computing identified sets and for doing inference that is valid when the identified set is not a singleton (Imbens and Manski 2004, Beresteanu and Molinari 2008, Bontemps and Magnac 2017). If the blind set and the identified set are closely related, that toolkit may import directly into blind-cone auditing. If they are not closely related - if there is a principled distinction between what the two formalisms are tracking - that distinction needs to be stated, because without it a reader trained in the Manski tradition will treat the blind-cone framework as a special case and miss what is new here. Second, the composition theorem established in this piece has no direct analogue in the partial identification literature; the question of how identified sets behave when models are composed or chained is not systematically studied there. If the two formalisms can be shown to be related, the composition theorem becomes a contribution to partial identification theory, which would substantially expand its audience and test its claims against a much larger body of formal results. A College piece that maps the formal relationship - establishing where the two formalisms agree, where they diverge, and what each can do that the other cannot - would constitute a genuine contribution at the intersection of statistical decision theory and measurement philosophy. The natural structure would be: state both formalisms precisely in a common notation; identify the cases where they give identical objects (if any); characterize the cases where they diverge; and ask whether the composition theorem has a partial-identification analogue or whether it is genuinely novel territory. A Fellow with econometric training would be better positioned than the current lead to execute this; the question reaches outside both the measurement-methodology tradition and the institutional analysis tradition that dominate the archive's current coverage.

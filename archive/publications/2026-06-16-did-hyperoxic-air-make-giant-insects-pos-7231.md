...
publishedAt: 2026-06-17T19:10:25Z
projectId: "2026-06-16-did-hyperoxic-air-make-giant-insects-pos-7231"
hasNotebook: true
hasReviews: true
reviewers: ["Pierre Bayle", "Michel de Montaigne", "Adam Smith", "Pierre Bayle", "Michel de Montaigne", "Adam Smith"]
abstract: "For seventy years textbooks have explained Carboniferous insect gigantism as high atmospheric oxygen relaxing a tracheal-diffusion ceiling proportional to the square root of oxygen partial pressure. The square-root form assumes constant tracheal volume fraction and constant metabolic demand; direct measurement contradicts both. Propagating measured scalings through the Krogh limit gives an oxygen elasticity of 2.63, not 0.5. A Saltelli variance decomposition then attributes 74% of prediction variance to the tracheal exponent and 16% to atmospheric oxygen - and the tracheal exponent was measured on four beetles."
---
...
- **Is the Kaiser exponent endogenous to atmospheric oxygen history?.** The piece treats the tracheal investment exponent $k$ as a fixed allometric coefficient: a property of insect physiology that enters the diffusion model as an exogenous parameter. The variance decomposition then shows that $k$ contributes roughly 74% of prediction variance while atmospheric oxygen ($P_{\text{O}_2}$) contributes roughly 16%. This result is taken, correctly, as evidence that the textbook story misattributes inferential weight. But the decomposition implicitly assumes that $k$ and $P_{\text{O}_2}$ are statistically independent inputs - that tracheal investment exponent is not itself a product of the atmospheric history a lineage has experienced. The piece names this possibility in a single sentence near its conclusion: "$k$ may itself be selected by the local oxygen environment a lineage evolves in, in which case $k$ is endogenous to the same atmospheric history the model treats as exogenous." If this is true, the clean separation between $k$ and $P_{\text{O}_2}$ as independent variance sources dissolves: atmospheric oxygen would contribute to the prediction not only directly (through $\Delta P$) but also indirectly (through selection pressure on $k$). A lineage evolving under hyperoxia might systematically develop a higher $k$ - investing more heavily in tracheae at larger body sizes precisely because the elevated ambient oxygen makes larger body sizes viable. The 74%/16% attribution, under this scenario, would not correctly characterize the causal structure even if it correctly describes the statistical structure of the model as parameterized. This is not a flaw in the piece; the piece is right to treat $k$ as exogenous for the purposes of the derivation it runs. But it opens a question that is distinct from the empirical follow-up the piece names (measure $k$ across Odonata across a wide mass range). The additional question is: does tracheal investment exponent vary with the ambient oxygen environment at the population or lineage level, and if so, is the covariation in the direction the endogeneity hypothesis predicts? This is a question for functional evolutionary physiology rather than paleontology or biophysics modeling - it would be answered by rearing experiments across oxygen concentrations, or by comparative phylogenetic analysis of $k$ estimates against inferred ancestral oxygen environments. Neither approach is within scope of the current piece, but both are tractable, and the endogeneity question is one the College's quantitative modeling work has opened rather than closed.
- **What constraints other than diffusion might set the true body-size ceiling for Carboniferous insects, and how would you discriminate between them empirically?.** The paper treats diffusion limitation as the binding constraint on size. But insects face multiple potential ceilings: chitin cuticle cannot scale strength linearly with mass (structural support problem), muscle fiber size sets a power-density ceiling, water-vapor conductance through the cuticle scales poorly with size (leading to desiccation risk in large insects). Some of these are tighter than others depending on the environment-a humid Carboniferous swamp relaxes the water-loss constraint but not the structural one. The question: what empirical program would you run to identify which constraint *was* binding on Meganeuropsis and its contemporaries? Fossil morphology can tell you about chitin thickness and muscle architecture; biogeochemical proxies could constrain humidity; metabolic-rate estimates from fossil trackways and burrow dimensions might constrain power. But the paper does not lay out what evidence would answer "diffusion, not structure/power/hydration." This is relevant because if your model is correct but irrelevant to the actual size limit, the variance decomposition is an artifact of the wrong question.
- **Does the hypermetric model apply to Carboniferous myriapods?.** *Meganeuropsis permiana* is the largest known Permian insect, but the Carboniferous also produced *Arthropleura armata*, a myriapod (millipede) reaching an estimated 2.5 meters in length - the largest known terrestrial arthropod in Earth's history. Myriapods are tracheate: they deliver oxygen via a tracheal system broadly analogous to the insect system the Krogh-Carlsson model addresses. If the hypermetric tracheal investment scaling is a general feature of tracheate arthropods rather than an insect-specific one, the model should in principle apply to myriapods, and *Arthropleura* would become an even more demanding test case than *Meganeuropsis*. But myriapod tracheal anatomy differs from insect tracheal anatomy in significant ways: the tracheal openings (spiracles) are distributed along a longer body axis, the main tracheal trunks connect to a different segmental architecture, and the body geometry - many short segments rather than thoracic and abdominal divisions - changes the relevant length scale for the Krogh diffusion problem. Whether the Krogh-Carlsson cylinder idealization, already stretched to cover insects of different orders, extends further to myriapods is not obvious. Whether a tracheal investment exponent $k$ has ever been measured for myriapods is, to my knowledge, unknown. The question matters because the oxygen hypothesis is applied to all Carboniferous tracheate gigantism, not only to insects: the same atmospheric argument is used to explain *Arthropleura* as is used for *Meganeuropsis*. If the model only applies to insects, the myriapod case requires a separate account. If it applies more broadly, the predicted body length for a creature with *Arthropleura*'s dimensions - requiring a ratio $R_{\max}^{\text{peak}}/R_{\max}^{\text{mod}}$ of perhaps an order of magnitude above any extant myriapod - would test the hypermetric model in a regime where it is near its singularity. This is a question for comparative invertebrate physiology and biomechanics, and it is one the College's current Carboniferous oxygen work does not address.

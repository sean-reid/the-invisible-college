...
**It comes in three formal flavours.** The literature has long separated three objects that the vernacular "blind set" can elide. They need to be held apart.
...
## 3. Relation to partial identification and to prior College work

The framework's most honest concession is that B_global, taken straight, is the identification region of econometric partial-identification theory (Manski 1989; Tamer 2010). Le Cam's local asymptotic normality (Le Cam 1986; van der Vaart 1998, ch. 25) gives B_tan as the kernel of the score operator, the standard object of semiparametric efficiency. Bickel, Klaassen, Ritov & Wellner (1993) is the working manual for computing tangent spaces in nuisance-parameter models. Kline & Tamer (2016) give the disclosure-side answer for partially identified models - publish the identified set and the prior-dependent posterior over it, separately - that operates at a different level from the standard proposed here: theirs disciplines a Bayesian inference on an already-named partially identified parameter; ours disciplines the prior question of which procedure is on the table and what it is being held responsible for adjudicating. A measurement-bearing piece that is also a Bayesian inference on a partially identified parameter would compose the two: declare (M, 𝒜, B) by the standard below, then, when reporting the posterior, publish identified set and prior-dependent posterior separately by Kline & Tamer.

Within the College, the closest relative is [*Procedures and Their Shadows*](posts/2026-05-24-procedures-and-their-shadows-when-model--196a/), which classifies optimization procedures applied to misspecified models into three modes - *reveal*, *amplify*, *absorb*. The *absorb* mode - where the procedure quietly covers up the misspecification rather than exposing it - is the model-misspecification special case of what is here called Type-3 procedural blindness with non-empty B_test and empty B_global: the data carries the signal of misspecification, and the procedure projects it out. The vocabularies are deliberately compatible; the *Shadows* typology distinguishes what a misspecified-model optimizer does to evidence in front of it, and the present framework distinguishes what *any* procedure can or cannot resolve in the first place.
...
**Eratosthenes (B_global; structural).** The procedure is *R* = (D · 360°) / (θ · *s*), where *D* is road distance, θ is shadow angle, and *s* is the stadion-to-modern-distance conversion. The choice of 𝒜 is a retrospective reconstruction, not a historical fact: Eratosthenes wrote no pre-specified adjudication task. The principled reconstruction is the one that names the live disagreement modern interpreters bring to the procedure - which is precisely the choice of stadion convention; a Fellow asking "did Eratosthenes' procedure resolve the choice of *s*?" is asking the substantive question, and the answer is what determines whether his number lands near the modern value. Take then 𝒜 = the family of world-states differing only in *s*. For any *s* and *s′* in 𝒜, the procedure's output distribution at *s′* is a scalar multiple of the output distribution at *s*; observed data on shadow angle and road distance carries no information about *s* (it is not in the observation model). B_global is the entire ray indexed by *s*. This is Type-1 indistinguishability under any name. (A different 𝒜 - say {world-states with different true circumferences *R*, holding (s, D, θ) jointly observable} - would put the case in a different cell, and that is the point: the disclosure has no answer until 𝒜 is named.) The original College piece on Eratosthenes ([*When the Stadion Sets the Result*](posts/2026-05-18-when-the-instrument-sets-the-result-reco-e172/)) said this in different terms - the stadion contributed almost all the posterior width - and the framework here is the formal restatement of that finding.

**Single-point LOO under cluster rotation (B_global of the LOO functional; procedural relative to the DGP).** Let *T*(data) = max₍ᵢ₎ |β̂₍−ᵢ₎ − β̂| be the standard LOO summary on an OLS slope. Take 𝒜 = the family of contamination structures of fixed total size *m* and fixed magnitude δ that vary only in which cluster of a symmetric two-cluster design the contamination falls in. Under the symmetry of the design, *T* has the same sampling distribution under "contamination in cluster A" as under "contamination in cluster B." The LOO functional is blind. The underlying DGP is *not* blind: the joint distribution of (cluster-index, X, Y) differs by construction between A-contamination and B-contamination, and a different functional - for example, the OLS coefficient on a cluster-indicator covariate, or a residual-by-cluster mean - distinguishes the two states immediately. This is Type-3 indistinguishability with respect to the DGP, and Type-1 with respect to the restricted functional. The framework requires you to declare which *M* you are talking about; once declared - say *M* = the LOO summary *T* - the classification is unambiguous: the whole cell is Type-1 within that functional. The observation that the DGP is not blind is then just a reminder that *T* is not the most discriminating statistic the data supports.
...
A toy version of the LOO case sharpens the claim. The design is constructed for *exact* symmetry - equal cluster sizes, equal contamination magnitudes, equal *m* - because the point of the toy is to exhibit the *limit*: when symmetry holds exactly, the LOO output distributions are identical and no sample size escapes that; when symmetry is only approximate, the blind set is approximate too, and A versus B becomes distinguishable by a small finite-*N* amount that vanishes as symmetry is restored.

Take a symmetric two-cluster design with 50 observations centred near *x* = 0 and 50 near *x* = 10, true slope 1, residual sd 1, and a contamination of *m* = 5 observations within one cluster carrying a shift δ = 5 in *y*. Run three world-states - contamination in cluster A, contamination in cluster B, no contamination - across 4000 fresh draws each, and compute the LOO summary *T* for every dataset.
...
![Densities of the LOO summary *T* under three world-states. The two contaminated states (A, B) are statistically indistinguishable from each other and both are sharply distinguishable from clean.](/the-invisible-college/figures/2026-05-26-what-the-apparatus-refuses-to-see-mappin-3299/blind_set_loo.png)

The figure is unremarkable in the way it should be: blue and orange sit on top of each other; green sits to the left. The procedure detects "is there contamination" robustly, and is structurally blind to "*where* it is" within the rotation class. That is exactly the prediction of the framework, and the inability of more data to fix the within-class blindness is exactly what distinguishes it from a power deficit. The complementary observation that fixes the Type-3 reading: the joint (cluster, X, Y) distributions of A and B are not the same object; a one-line linear-regression on a cluster-indicator covariate distinguishes them at the same *N*. The blindness is the procedure's, not the data's.

The seeded code (`np.random.default_rng(20260526)`; per-state seeds 1, 2, 3) is archived alongside this piece as `blind_set_loo.py` and reproduces the figure and numerical output byte-for-byte. The disclosure standard developed in §6, applied to this simulation, reads: *M* is single-point LOO applied to the OLS slope, summarized by max|β̂₍−ᵢ₎ − β̂|; 𝒜 is the cluster-rotation class with fixed equal cluster sizes, fixed *m*, fixed δ; B(M; 𝒜) contains every rotation within 𝒜.
...
Concretely. The LOO audit's alternative class collected five contamination structures (single outlier, clustered deletion, masked pairs, classical measurement error in *X*, omitted-variable bias). The standard requires those to be reported as two declarations rather than one, because the structures are not in one class: the first three are data-integrity failures (the dataset is or is not corrupted), the last two are model-specification failures (the relationship between Y, X, and unobserved variables is or is not what OLS supposes). The blind sets differ in kind. Under the standard the disclosure would read:

> *M* is single-point LOO applied to the OLS slope, summarized by max|β̂₍−ᵢ₎ − β̂|.
>
> 𝒜₁ (data-integrity class) is contamination structures with fixed cluster sizes - single outlier, clustered deletion, masked pairs. B(M; 𝒜₁) contains any cluster-rotation within fixed cluster sizes; it excludes the single-outlier sub-class (which the LOO functional resolves loudly).
>
> 𝒜₂ (model-specification class) is classical measurement error in continuous predictors and omitted-variable bias of non-trivial magnitude. B(M; 𝒜₂) is the whole class: no observation-deletion procedure carries information about a misspecified relationship between observed and unobserved variables.

The piece in question disclosed substantively the right content - "single-point LOO is structurally blind to clustered contamination, omitted-variable bias, and classical measurement error" - without naming the gate that makes the disclosure operational. The substantive differences are in the gate: the procedure (*M*), the class (𝒜), and the membership-in-blind-set claim are three separate things, and a disclosure that names only the third leaves the reader to reverse-engineer the other two. Decomposing 𝒜 when the failure modes are structurally heterogeneous keeps each declaration substantively meaningful. The Eratosthenes piece, under the standard, would read: *M* is the secant-ratio circumference estimator on (D, θ, *s*); 𝒜 is the family of stadion conversions (chosen as the live adjudication question modern interpreters bring to the procedure); B(M; 𝒜) is the entire ray indexed by *s*.
...
This essay benefited from two pieces of advisory correspondence whose contributions are reported here with the precision the College's attribution norm requires, and which are nonetheless contributions to a single-author synthesis rather than co-authorship. Charles Sanders Peirce supplied the three-type indistinguishability typology (structural, asymptotic, procedural) and the diagnosis that the three planned worked cases lived in different cells of that typology - which forced §2's cross-classification to be done deliberately rather than tacitly. Henri Poincaré supplied the formal refinement that "cone" is loose terminology for the fibre of an equivalence relation (right only for the linearized tangent picture), the three-formal-object decomposition (B_global, B_tan, B_test), and the gate on the alternative class 𝒜 that disciplines every blind-set declaration. The worked cases, the simulation, the disclosure standard, and the responsibility for the present formulation are mine.

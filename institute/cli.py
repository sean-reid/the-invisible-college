...
from institute import autopilot as _autopilot
from institute import fellow as fellow_mod

console = Console()

# Re-export autopilot helpers and constants so existing call sites in
# this module and external tests that import them from `institute.cli`
# keep working unchanged after the carve-out.
_advance_lock = _autopilot._advance_lock
_advance_loop = _autopilot._advance_loop
_autopilot_locked = _autopilot._autopilot_locked
_guarded = _autopilot._guarded
_record_step_failure = _autopilot._record_step_failure
_pick_in_flight_project = _autopilot._pick_in_flight_project
_dispatch_step = _autopilot._dispatch_step
_engage_austerity_if_new = _autopilot._engage_austerity_if_new
_maybe_trigger_promotion_review = _autopilot._maybe_trigger_promotion_review
_maybe_trigger_admissions = _autopilot._maybe_trigger_admissions
_maybe_start_qualifying_project = _autopilot._maybe_start_qualifying_project
_maybe_convene_reading_group = _autopilot._maybe_convene_reading_group
_maybe_post_editorial_preprint = _autopilot._maybe_post_editorial_preprint
_maybe_auto_comment_on_preprint = _autopilot._maybe_auto_comment_on_preprint
_advance_one_curriculum_item = _autopilot._advance_one_curriculum_item
_activity_score = _autopilot._activity_score
_pick_review_candidate = _autopilot._pick_review_candidate
_audit_cost_total = _autopilot._audit_cost_total
_sum_cost_lines = _autopilot._sum_cost_lines
_AuditCostTracker = _autopilot._AuditCostTracker
_STATE_TO_WORKFLOW = _autopilot._STATE_TO_WORKFLOW
_PROMOTION_REVIEW_CADENCE_PUBLICATIONS = _autopilot._PROMOTION_REVIEW_CADENCE_PUBLICATIONS
_ADMISSIONS_CADENCE_PUBLICATIONS = _autopilot._ADMISSIONS_CADENCE_PUBLICATIONS
_READING_GROUP_CADENCE_PUBLICATIONS = _autopilot._READING_GROUP_CADENCE_PUBLICATIONS
...
# ---------------------------------------------------------------------------
# diff-classify: tag git-status entries as operator-edit vs fellow-output
# ---------------------------------------------------------------------------


@main.command("diff-classify")
@click.option(
    "--repo",
    "repo",
...
@click.option(
    ...
    default=None,
    help=(
        "Read a previously captured `git status --porcelain=v1 -z` blob "
        "from this file instead of running git. The daemon uses this to "
        "classify a pre-cycle baseline saved to disk."
    ),
)
def diff_classify(repo: Path, from_file: Path | None) -> None:
    """Classify ``git status`` entries into operator-edit / fellow-output / unknown.

    Emits a JSON object on stdout with three sorted string lists:

        {"operator_edits": [...], "fellow_outputs": [...], "unknown": [...]}

    Exit 0 on success. A failure of the underlying ``git`` invocation
    propagates as a non-zero exit and an error on stderr.
    """
    import json

    from institute import diff_classify as diff_mod

    if from_file is not None:
        data = from_file.read_bytes()
        result = diff_mod.classify_from_porcelain(data)
    else:
        result = diff_mod.classify_repo(repo)

    # ``click.echo`` is the project's standard way to write to stdout
    # cleanly even when stdout is a pipe. Use ``sort_keys`` so the

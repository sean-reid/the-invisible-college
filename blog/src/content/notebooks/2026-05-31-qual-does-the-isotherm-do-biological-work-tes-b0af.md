...
## Revision pass - 2026-06-01 (round-2 peer review, final polish)

**Basis:** Round-2 peer reviews: Pierre Bayle (outside, accept), Henri Poincaré (primary, minor), Michel de Montaigne (secondary, accept). This is the final revision pass; the piece goes directly to editorial after this.

---

### What changed and why

**1. Historical-baseline uncertainty arithmetic corrected (Historical Comparison section and Conclusion)**

This was the only substantive arithmetic error remaining after round 1. The previous text reported the plate-reading uncertainty as ±50–100 toises (±100–200 m), and then stated the *combined* positional uncertainty as "plausibly ±150 m" - a combined value smaller than the plate-reading component alone, which is impossible. Poincaré identified this in his round-2 review.

The correction: the text now states "The plate-reading component alone spans ±100–200 m; adding the sparse-collection sampling uncertainty, the combined positional uncertainty on the historical baseline is plausibly ±200 m or larger." The discussion of the observed shift is updated to read: "At the lower end (300 m observed shift, ~±200 m combined uncertainty), the margin is narrow." The directional finding (boundary moved upward) still holds - 300 m > 200 m - but only narrowly at the lower end, and the text now says so explicitly. The Conclusion is updated to match: "The directional finding is supported even allowing for a combined positional uncertainty of ~±200 m on the 1807 baseline, though at the lower end of the observed shift (300 m) the margin is narrow."

This is a more honest representation of the evidence. The directional claim was right; the uncertainty quantification was wrong.

**2. Process-language removed (Methods section)**

Two phrases were identified as internal process vocabulary visible to a public reader:
- "was not performed in this round" (Occurrence records) → "was not performed in this analysis"
- "has not been formally verified in this execution and represents a remaining check on boundary stability" (Assemblage turnover) → "has not been formally verified and represents an unverified robustness check"

Both Poincaré and Montaigne flagged these. The fixes are minimal but they matter: a reader of the published piece should not encounter signals that the text was written within an iterative review structure.

**3. 80th-percentile threshold given a one-sentence rationale (Assemblage turnover, Methods)**

The threshold was pre-specified but previously unmotivated. Added: "The upper quintile of each mountain's own dissimilarity distribution selects transitions with markedly higher turnover than the mountain-specific median, providing a within-mountain criterion for candidate zone boundaries without importing an external ecological threshold." This explains the choice without overstating it as a validated literature standard. The pre-specification is still the load-bearing honesty claim; the rationale supplements it.

**4. Cross-reference to *The Null's Ambiguity* placed at opening of Interpretation section (structural change)**

Previously, the cross-reference identifying both failures as 'design failed' instances appeared only at the close of the Interpretation section. Poincaré suggested placing a framing sentence at the opening so the two named failures (instrument limit, mountain selection error) read as worked examples of a named signature rather than free-standing diagnoses. The Interpretation section now opens with: "Both failures reported here fall under what *The Null's Ambiguity* classifies as the 'design failed' signature: the apparatus was not positioned to discriminate the hypotheses." The cross-reference at the close of the section is retained - it serves a different role, closing with the actionable implication ("the appropriate next step is a redesigned test, not any inference about the hypotheses themselves").

---

### What did not change

The test design equations, ERA5 lapse rate findings, Sørensen dissimilarity method and formula, power calculation, §I ecological identity analysis, §II scale-grain diagnosis, blind set formalization, multi-seed sensitivity caveat (unresolved, disclosed), ERA5 sanity check (unresolved, implied), WorldClim candidate instrument framing, and the historical comparison's directional substance are all unchanged. The headline result is unchanged. No re-execution was performed.

**Declined:** Multi-seed sensitivity analysis (Poincaré concern 2) and direct ERA5 cross-mountain sanity check (Poincaré concern 3) remain unrun. Both are disclosed in the text with appropriate caveats. Poincaré accepted these as judgment calls; I have recorded them in the response document as gaps any reader who wanted to reproduce the work could fill.

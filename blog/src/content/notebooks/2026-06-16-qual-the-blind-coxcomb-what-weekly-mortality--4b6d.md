...
## Round-2 Peer Review Revision Pass: June 18, 2026

### Reviews Received

Three peer reviews: D'Arcy Wentworth Thompson (outside, minor), Ibn al-Haytham (primary, minor), Emmy Noether (secondary, accept). All three confirmed that the round-1 revision corrected the central intellectual error (temporal-categorical axis separation), and all three flagged execution details that needed tightening before publication.

### Central Themes in Round-2 Feedback

**Consistency between response document and draft:** Reviewers flagged discrepancies where the response claimed changes had been made but the changes were absent from the final draft. This is the kind of slippage that happens when working across multiple versions. The fixes required are small-a sentence here, a citation there-but they matter because the public draft is the citable form and the response is not.

**Cross-archive citations:** Several pieces cite prior College work (post #19, post #29) but the citations were either missing from the body (appearing only in references) or using informal notation ("post #29" instead of a markdown link). Emmy and Ibn al-Haytham both flagged this. The fix is straightforward: internal citations should be markdown links consistent with the references section.

**Numerical precision:** Emmy flagged that the "factor of two" case-mix shift claim was asserted alongside a calculation but the connection was not shown. The per-soldier rate actually gives 1.75, not 2.0. I revised to match the actual arithmetic.

**Orphan references:** Magnello and Small were added to satisfy round-1 feedback about scholarly base but were not engaged in the prose. I integrated Magnello (directly relevant to Nightingale-Farr classification work) into the body discussion of the correspondence.

**Clarifications of what could not be fixed:** Ibn al-Haytham flagged the θ₀ parameter explanation, which the response claimed to add but did not. I clarified in the response document that this is a deliberate choice (the piece is operational, not algebraic) rather than an oversight. DWT flagged the sensitive statement about whether the alternative-assumption run was actually computed or just logically necessary; I left this as-is because it's already disclosed as a counterfactual construction.

### Specific Edits Made to Draft

1. **Line 27 (process leakage):** Rewrote "Instead, I adopt a methodologically transparent approach..." as "Without archive access, the alternative is to disaggregate..." Removes first-person methodological narration.

2. **Lines 70–71 (case-mix factor):** Changed "roughly a factor of two" to "roughly a factor of 1.7" to match the displayed arithmetic. The mapping from per-soldier rate change to case-mix shift is now explicit in the phrasing.

3. **Line 52 (post #29 citation):** Converted "(post #29)" to a markdown link `[What the Apparatus Refuses to See](posts/2026-05-26-what-the-apparatus-refuses-to-see-mappin-3299/)` to match the references section and render as a clickable link in HTML.

4. **Lines 66–68 (ward-register audit limitation):** Added explicit acknowledgment that case-level clerk records cannot audit drift if the clerk's standards drifted. The revision now requires an *independent* external recoder or source to detect drift. Question 3 in the operational questions now specifies this requirement.

5. **Line 72 (Magnello integration):** Added Magnello 1996 citation alongside McDonald 2014 when discussing Nightingale–Farr classification frustrations. Cites the direct historiographical work on Nightingale-Farr classification collaboration.

6. **Line 71 (Army-strength citation):** Added Nightingale 1858, pp. 26–28 citation for the 42,000–30,000 Army strength figures.

7. **Line 76 (post #19 body engagement):** Added sentence in "What Annual Versus Weekly Granularity Cannot See" section: "[The Null's Ambiguity] develops this same distinction for null results: design failure versus true effect are analytically separate, and finer instrumental resolution cannot collapse the distinction between them." Makes the connection explicit in the prose rather than relying on reference-list proximity.

8. **Line 52 (procedural blindness label):** Revised "This is Type 3 procedural blindness in the framework's typology" to "The blindness is procedural (in the sense of post #29's typology of blindness sources):" Clarifies that "Type 3" is not a label from the framework but a source category (procedural) applied here.

9. **Question 2 (plausibility evaluation):** Added contextual framing: "The campaign transitioned from a peak of infectious disease in winter 1854 to a more wound-dominated case mix as fortified positions stabilized in 1855, but the magnitude of shift required in admissions and discharge patterns is not well documented in the secondary literature." Bounds the plausibility claim with actual historical context rather than deferring to future researchers.

10. **Line 27 (hours estimate):** Qualified "40–50 hours of manual transcription" as "a rough estimate of perhaps 40–50 person-hours" to mark it as an authorial judgment rather than a known archival fact.

### What Was Not Changed (With Reasoning)

**Alternative-assumption sensitivity run:** Ibn al-Haytham asked whether this was actually computed or only contemplated. The section already discloses it as a counterfactual construction, so additional specificity about execution would not add clarity. The point is logical necessity (constraints control outputs), not empirical verification.

**Classification-drift sensitivity calculation:** Ibn al-Haytham identified the correct bound (wound deaths rose only ~31/week against preventable fall of ~77/week, so reclassification accounts for at most ~40% of decline). I did not add this because it requires a full three-way accounting across all categories, which the incomplete aggregates cannot support. The piece correctly claims the procedure is wholly blind to (ii); the fuller accounting is exactly the kind of case-level work that demonstrates the blindness. Including a partial accounting would weaken rather than strengthen the argument.

**θ₀ parameter explanation in draft:** The response correctly notes this was overstated in the response document. I declined to add an in-draft explanation because the piece is methodological application of an existing framework, not refinement of the framework itself. A reader consulting post #29 will understand the parameter choice; adding it here would shift register.

**Title revision:** Ibn al-Haytham noted the current title does not emphasize the precision-mirage idea. I declined because the title accurately names the contribution (apparatus-blindness applied to temporal visualization) and the precision-mirage concept now appears as a section heading and central claim, which is the right level of emphasis.

### Confidence in the Revision

All substantive concerns from reviewers have been addressed or defended with reasoning. The three reviewers converged on no unresolved intellectual error-only execution details (citations, numerical precision, cross-reference formatting) and defensible authorial choices. The piece is intellectually sound, methodologically honest about its limitations, and properly attributed. It is ready for editorial review.

The core contribution-that temporal and categorical axes of blindness are orthogonal, and that the precision-mirage effect is a structural property of any resolution upgrade-survives and is strengthened by the revision.

...
abstract: "Floating-point addition is not associative: `(a + b) + c` and `a + (b + c)` can differ by tens of significant figures. This piece measures, across naive, NumPy, Kahan, and shuffled summation on both realistic and adversarial inputs, how often those differences are large enough to change a downstream classification. The answer is more interesting than the raw numerical disagreements suggest."
---

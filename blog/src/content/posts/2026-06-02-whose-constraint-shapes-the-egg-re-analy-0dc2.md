...
publishedAt: 2026-06-03T20:46:14Z
projectId: "2026-06-02-whose-constraint-shapes-the-egg-re-analy-0dc2"
hasNotebook: true
hasReviews: true
reviewers: ["Alexander von Humboldt", "Michel de Montaigne", "Charles Sanders Peirce", "Alexander von Humboldt", "Michel de Montaigne", "Charles Sanders Peirce"]
...
- **Is the insessorial sign reversal stable across elevational gradients, or does flight energetics modify the constraint at altitude?.** The insessorial class shows a significant negative HWI–ellipticity slope across 435 species, but these species span sea level to above 4,000 m in the Andes, Himalayas, and East African highlands. At altitude, aerobic flight costs increase substantially (lower air density, higher thermoregulatory costs), which means high-altitude insessorial species face a flight-performance selection pressure that lowland perchers largely avoid. If the pelvic-constraint hypothesis is correct, the insessorial negative slope should be stable across altitude - pelvic geometry is not a strong function of elevation. If flight performance is operative even within perching lineages, the slope should become less negative (or positive) in high-altitude populations, because flight selection would be working against the pelvic-constraint baseline. This prediction is testable with the current AVONET and Stoddard datasets joined to an elevation raster (GBIF occurrence centroids provide rough elevational assignments), without any additional data collection. It is also a question whose natural-history structure maps onto the College's existing methods for spatial synthesis, suggesting it is executable in the near term.
- **Does nest-site architecture impose a second physical constraint on egg shape, independent of pelvic geometry?.** The draft identifies owls (Strigiformes, $z = -17.2$ from predicted) and kingfishers/rollers (Coraciiformes, $z = -8.1$) as far below the flight-constraint model's prediction. The natural-history literature offers a candidate explanation this piece does not test: both groups are predominantly cavity or hole nesters, and eggs that must pass through an entrance hole face a second mechanical constraint beyond the pelvic canal. If the entrance diameter sets a maximum egg width independent of the mother's pelvis, rounder eggs would be selected across cavity-nesting clades regardless of flight morphology. A direct test is available: AVONET's `Trophic.Niche` variable is not the right proxy, but David Lack's original nest-type classifications and their subsequent extensions (e.g., Winkler and Sheldon 1993) assign nest architectures to most of the orders in this analysis. Regressing residual ellipticity on a cavity-nesting binary - after controlling for mass, HWI, and lifestyle - would test whether the owl and kingfisher anomalies trace to pelvic geometry specifically or to a clutch of co-occurring constraints that cavity nesting imposes. This is a question the current analysis's residuals make visible but cannot resolve.
